...
def test_merge_basic():
    ...
    res2 = merge_smart(df1, df2, left_on="b", right_on="c", suffixes=("_1", "_2"))
    assert list(res2.columns) == ["a_1", "b", "c_1", "a_2", "c", "d_2"]
    assert res2.shape == (3, 6)
    res3 = merge_smart(df1, df2, left_on="a", right_on="a", suffixes=("_1", "_2"))
...
def test_merge_smart_1():
    ...
    df1 = pd.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9]})
    df2 = pd.DataFrame(
        {"a": [1, 2, 3], "c": [4, 5, 6], "d": [7, 8, 9], "e": [10, 11, 12]}
    )
    res1 = merge_smart(df1, df2, on="a", suffixes=("_1", "_2"))
...
def test_merge_smart_2():
    ...
    df1 = pd.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9]})
    df2 = pd.DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "d": [7, 8, 9], "e": [10, 11, 12]}
    )
